...
from modules.data.kis_api import (
    get_kis_daily_data,
    get_kis_intraday_data,
    get_secret_value,
    get_stock_code,
    is_kis_configured,
    is_kis_domestic_supported,
)

from modules.data.yfinance_api import (
    get_yfinance_news,
    normalize_yfinance_data,
    safe_yfinance_download,
)
...
def calculate_intraday_signal(df, interval="-"):
    ...
    def add_score(item, score, comment, weight=1.0):
        ...
        score_100 = score_10_to_100(raw_score)
        score_rows.append(
            {
                "평가 항목": item,
                "점수": score_100,
                "가중치": weight,
                "판정": score_to_intraday_signal(score_100),
                "해석": comment,
            }
        )
        comments.append(f"{item}: {comment} ({score_100}/100)")
    ...
    flow_score = clamp_score_100(weighted_score)
...
def is_positive_attention_condition(score):
    return pd.notna(score) and score >= 61


def is_negative_caution_condition(score):
    return pd.notna(score) and score <= 39
...
def score_10_to_100(score):
    if score is None or pd.isna(score):
        return 50
    raw_score = clamp_score(score)
    return clamp_score_100(((raw_score - 1) / 9) * 100)
...
def get_signal_badge_class(score):
    return get_signal_badge_class_100(score)
...
def calculate_signal(df):
        ...
        score_rows.append(
            {
                ...
                "점수": score_100,
                "가중치": weight,
                "판정": score_to_signal(score_100),
                "해석": comment,
            }
        )
        reasons.append(f"{item}: {comment} ({score_100}/100)")
    ...
    if pd.notna(today["ADX"]) and pd.notna(today["Plus_DI"]) and pd.notna(today["Minus_DI"]):
        if today["ADX"] >= 30 and today["Plus_DI"] > today["Minus_DI"]:
            adx_score = 9
            adx_comment = "ADX가 30 이상이고 +DI가 -DI보다 높아 상승 방향 추세 강도가 강하게 나타납니다."
        elif today["ADX"] >= 30 and today["Minus_DI"] > today["Plus_DI"]:
            adx_score = 2
            adx_comment = "ADX가 30 이상이고 -DI가 +DI보다 높아 하락 방향 추세 강도가 강하게 나타납니다."
        elif today["ADX"] >= 25 and today["Plus_DI"] > today["Minus_DI"]:
            adx_score = 7
            adx_comment = "ADX가 25 이상이고 +DI가 -DI보다 높아 상승 방향성이 우세하게 나타납니다."
        elif today["ADX"] >= 25 and today["Minus_DI"] > today["Plus_DI"]:
            adx_score = 3
            adx_comment = "ADX가 25 이상이고 -DI가 +DI보다 높아 하락 방향성이 우세하게 나타납니다."
        elif today["ADX"] >= 18 and today["Plus_DI"] > today["Minus_DI"]:
            adx_score = 6
            adx_comment = "ADX가 18 이상이고 +DI가 -DI보다 높아 상승 방향성이 일부 나타납니다."
        elif today["ADX"] >= 18 and today["Minus_DI"] > today["Plus_DI"]:
            adx_score = 4
            adx_comment = "ADX가 18 이상이고 -DI가 +DI보다 높아 하락 방향성이 일부 나타납니다."
        elif today["ADX"] < 18:
            adx_score = 5
            adx_comment = "ADX가 낮아 뚜렷한 방향성보다 횡보 가능성이 큽니다."
    ...
    category_details = score_details.copy()
    category_details["카테고리"] = category_details["평가 항목"].apply(get_condition_category)
    category_scores = []
    for category, category_weight in category_weights.items():
        category_rows = category_details[category_details["카테고리"] == category]
        if category_rows.empty:
            category_score = 50
        else:
            category_score = np.average(category_rows["점수"], weights=category_rows["가중치"])
        category_scores.append(category_score * category_weight)
    technical_score = clamp_score_100(sum(category_scores))
...
def get_condition_category(item):
    ...
    if item_text in exact_map:
        return exact_map[item_text]
...
def render_technical_score_breakdown(signal_result):
    ...
    cards = [
        ("종합 기술 조건 충족도", f"{signal_result['technical_score']} / 100", "기술적 조건 참고"),
    ]
    for _, row in summary_df.iterrows():
        value = f"{row['점수']:.1f} / 100" if pd.notna(row["점수"]) else "-"
...
def render_intraday_summary(df, result):
    ...
    cards = [
        ("당일 흐름 조건 충족도", f"{result['flow_score']} / 100", "일봉 조건과 별도"),
        (
            "당일 흐름 판단",
...
def calculate_watchlist_score(daily_result, intraday_result, latest, disclosures):
    daily_score = daily_result.get("technical_score")
    intraday_score = intraday_result.get("flow_score")
    daily_score = daily_score if daily_score is not None and pd.notna(daily_score) else 50
    intraday_score = intraday_score if intraday_score is not None and pd.notna(intraday_score) else 50
    volume_score = score_10_to_100(latest.get("_volume_score", 5))
    volatility_score = score_10_to_100(get_volatility_score(daily_result.get("risk_level")))
    total_score = (daily_score * 0.5) + (intraday_score * 0.3) + (volume_score * 0.1) + (volatility_score * 0.1)

    return clamp_score_100(total_score)


def classify_watchlist_candidate(row):
    daily_score = row.get("일봉 기술 조건 충족도", 0)
    intraday_score = row.get("당일 흐름 조건 충족도", 0)
    ...
    if pd.notna(rsi) and rsi >= 70:
        return "과열 주의"
    if daily_score >= 61 and intraday_score >= 61:
        return "당일 흐름 우세 후보"
    if daily_score >= 61 and pd.notna(rsi) and 45 <= rsi <= 65:
        return "상승 추세 후보"
    if daily_score >= 40 and pd.notna(rsi) and rsi <= 35:
        return "눌림목 점검 후보"
...
def analyze_watchlist_ticker(ticker, daily_period, intraday_interval, dart_api_key, data_source_preference="auto"):
    ...
    try:
        ...
        row = {
            ...
            "분석 코드": normalized_ticker,
            "자산 유형": asset_meta["asset_type"],
            "일봉 기술 조건 충족도": daily_result["technical_score"],
            ...
            "당일 흐름 조건 충족도": intraday_result.get("flow_score") if intraday_result.get("flow_score") is not None else np.nan,
...
def render_watchlist_priority_cards(result_df):
    ...
    for _, row in valid_df.iterrows():
        ...
        card_html += (
            ...
            f"<div class='score'>{row['종합 점검 조건 충족도']} / 100</div>"
            "<div class='meta'>"
...
def render_watchlist_scanner(dart_api_key, data_source_preference="auto"):
    ...
    if analyze_watchlist:
        ...
        if not assets:
            ...
            result_df = result_df.sort_values("종합 점검 조건 충족도", ascending=False).reset_index(drop=True)
